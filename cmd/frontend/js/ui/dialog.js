...
export default class Dialog extends HTMLElement {
	...
	constructor() {
		...
		this._gridTemplateRows = [ "2em", "auto", "2em" ];
	...
	set type(type) {
		...
		case "confirm":
			...
			this._dom.btnCancel.textContent = "Cacel";

			this._gridTemplateRows[2] = "2em";
			break;

		case "save":
			this._dom.footer.style.display = "flex";
			this._dom.btnOk.style.display = "block";
			this._dom.btnOk.textContent = "Save";
	...
	set title(title) {
		...
		this._gridTemplateRows[0] = title ? "2em" : "0";
		this.style["grid-template-rows"] = this._gridTemplateRows.join(" ");
	}
